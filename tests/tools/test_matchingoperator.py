...
def test_select_matching_operator_none():
    ...
    assert select_mo(None, 8) == MO.IGNORE
    assert select_mo(None, 16) == MO.IGNORE
    assert select_mo(None, 32) == MO.IGNORE
...
def test_select_matching_operator_match_mapping():
    ...
    assert select_mo(mapping, 8) == MO.MATCH_MAPPING
    assert select_mo(mapping, 16) == MO.MATCH_MAPPING
    assert select_mo(mapping, 32) == MO.MATCH_MAPPING
...
def test_select_matching_operator_buffer():
    ...
    assert select_mo(short_buffer, 16) == MO.MSB
    assert select_mo(short_buffer, 32) == MO.MSB

    # Test with Buffer equal to field length
    equal_buffer = Buffer(b"\x01\x02")  # 16 bits
    assert select_mo(equal_buffer, 16) == MO.EQUAL

    # Test with Buffer longer than field length
    long_buffer = Buffer(b"\x01\x02\x03\x04")  # 32 bits
    assert select_mo(long_buffer, 8) == MO.IGNORE
    assert select_mo(long_buffer, 16) == MO.IGNORE
